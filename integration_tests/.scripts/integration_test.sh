...
for db in ${DATABASES[@]}; do
  ...
  echo_violet "Snowplow unified integration tests: Seeding data"
  eval "dbt seed --full-refresh --target $db" || exit 1;

  echo_violet "Snowplow unified integration tests: Try run without data"
  ...
  echo_violet "Snowplow unified integration tests: Try run without data for browser_context_2 only enabled"
  ...
  echo_violet "Snowplow unified integration tests: Try run without data for browser_context_2 and browser_context enabled"
  ...
  echo_violet "Snowplow unified integration tests: Conversions"
  ...
  echo_violet "Snowplow unified integration tests: App errors module"
  ...
  echo_violet "Snowplow unified integration tests: Late enabled contexts"
  ...
  echo_violet "Snowplow unified integration tests: Late enabled contexts test passed"

  echo_violet "Snowplow unified integration tests: Execute models (all contexts except for cwv) - run 1/4"
  ...
  for i in {2..4}
  do
    echo_violet "Snowplow unified integration tests: Execute models (all contexts except for cwv) - run $i/4"
    eval "dbt run --vars '{snowplow__enable_cwv: false}' --target $db" || exit 1;
  done

  echo_violet "Snowplow unified integration tests: Test models"
  ...
  echo_violet "Snowplow unified integration tests: All non-CWV tests passed"
  
  echo_violet "Snowplow unified integration tests - Core Web Vitals: Execute models"
  ...
  echo_violet "Snowplow unified integration tests: Execute web (all web contexts except for cwv)"
  ...
  echo_violet "Snowplow unified integration tests: Execute mobile (all mobile contexts)"
  eval "dbt run --full-refresh --vars '{snowplow__allow_refresh: true, snowplow__backfill_limit_days: 9999, snowplow__enable_web: false, snowplow__enable_iab: false, snowplow__enable_ua: false, snowplow__enable_browser_context: false, snowplow__enable_browser_context_2: false, snowplow__enable_consent: false, snowplow__enable_cwv: false}'  --select +snowplow_unified_users snowplow_unified_events_stg --target $db" || exit 1;

  echo_violet "Snowplow unified integration tests: All CWV tests passed"

  echo_violet "Snowplow unified integration tests: Test mobile screen engagement"
  ...
  echo_violet "Snowplow unified integration tests: Mobile screen engagement tests passed"
