...
with session_firsts as (
    select
        ...
        {% if var('snowplow__session_stitching') %}
          ...
          , cast(user_identifier as {{ type_string() }}) as stitched_user_id
        {% else %}
          , cast(null as {{ type_max_string() }}) as stitched_user_id
        {% endif %}
...
{%- if var('snowplow__conversion_events', none) %}
left join
    session_convs d on f.session_identifier = d.session_identifier
{%- endif %}
